...
const DEFAULT_WALLET_SESSION_KEY = "liberdus-wallet-module:walletSession";
...
function isPhantomNamespaceProvider(provider) {
  ...
  const namespaceProvider = window.phantom?.ethereum;
  if (!namespaceProvider || !provider) return false;

  const namespaceProviders = Array.isArray(namespaceProvider.providers) ? namespaceProvider.providers : [];
  const candidateProviders = Array.isArray(provider.providers) ? provider.providers : [];
...
function createWalletCore({
  ...
  function syncWalletEventProvider() {
    ...
    boundAccountsChanged = async (accounts) => {
      const nextAddress = normalizeAddress(Array.isArray(accounts) ? accounts[0] : accounts);
  ...
  function initWalletDiscovery() {
    ...
    window.addEventListener(EIP6963_ANNOUNCE_EVENT, (event) => {
      ...
      if (wallet) {
        if (discoveredWallets.size && wallet.source === "eip6963") {
          syncWalletEventProvider();
        }
      }
    });
  ...
  function applyActiveWallet(wallet) {
    ...
    state.selectedWalletId = wallet?.id || null;
    state.selectedWalletName = wallet?.info?.name || null;
    state.selectedWalletRdns = wallet?.info?.rdns || null;
    state.injectedProvider = nextInjectedProvider;
    state.providerSource = nextInjectedProvider;
    syncWalletEventProvider();
  }

  function getInjectedProviderForRuntime() {
    const selectedWallet = state.selectedWalletId ? findWalletById(state.selectedWalletId) : null;
    if (selectedWallet) {
      state.selectedWalletName = selectedWallet.info?.name || null;
      state.selectedWalletRdns = selectedWallet.info?.rdns || null;
      state.injectedProvider = selectedWallet.provider || null;
      return state.injectedProvider;
    }
    return state.injectedProvider || getReadOnlyInjectedProvider();
  }
  ...
  async function ensureInjectedProvider() {
    const injected = getInjectedProviderForRuntime();
    if (!injected) {
  ...
  async function connect({ walletId, config = null } = {}) {
    ...
    try {
      applyActiveWallet(wallet);
      const provider = await ensureInjectedProvider();
      ...
      emitEvent("connected", { walletId: wallet.id, account: state.account, chainId: state.chainId });
  ...
  async function sync(config = null) {
    ...
    applyActiveWallet(selectedWallet);
    const injected = getInjectedProviderForRuntime();
    ...
    try {
      ...
      if (!account) {
        ...
        state.sessionWalletId = session.walletId;
      }
    } catch {
      state.account = null;
      clearWalletSession();
  ...
  return {
    ...
    getState,
    subscribe,
    getEip1193Provider: () => getInjectedProviderForRuntime(),
    getWalletSessionKey: () => walletSessionKey,
  };
}

export { createWalletCore, DEFAULT_WALLET_SESSION_KEY };
